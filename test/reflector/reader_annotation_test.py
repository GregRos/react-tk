...
class Reader_Annotation_Test:
    ...
    def it_has_right_inner_type(self, target, expected):
        ann = self.r.annotation(target)
    ...
    def it_has_right_name(self, target, expected):
        ann = self.r.annotation(target)
    ...
    def it_has_correct_inner_class(self, target, expected):
        ann = self.r.annotation(target)
        assert ann.inner_class == self.r.type(expected)
    ...
    def it_has_metadata(self, target, expected):
        ann = self.r.annotation(target)
    ...
    def it_filters_metadata_by_type(self, target, types, expected):
        ann = self.r.annotation(target)
        got = tuple(x for x in ann.metadata_of_type(*types))
        assert got == tuple(self.r.annotation(x) for x in expected)
    ...
    def it_stringifies_correctly(self, target, expected):
        ann = self.r.annotation(target)
    ...
    def it_has_right_origin(self, target, expected):
        ann = self.r.annotation(target)
        assert ann.origin == self.r.annotation(expected)
